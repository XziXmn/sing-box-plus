...
[[ -f "$is_sh_dir/src/relay.sh" ]] && . "$is_sh_dir/src/relay.sh"
...
uninstall() {
    ...
    rm -rf $is_core_dir $is_log_dir $is_sh_bin ${is_sh_bin/$is_core/sb} /usr/local/bin/sbb
    if [[ $is_systemd ]]; then
...
get() {
    ...
    protocol)
        ...
        socks*)
            net=socks
            is_protocol=$net
            [[ ! $is_socks_user ]] && is_socks_user=singboxplus
...
info() {
    ...
    ws | tcp | h2 | quic | http*)
        if [[ $host ]]; then
            ...
            [[ $is_protocol == 'vmess' ]] && {
                is_vmess_url=$(jq -c '{v:2,ps:'\"sing-box-plus-$net-$host\"',add:'\"$is_addr\"',port:'\"$is_https_port\"',id:'\"$uuid\"',aid:"0",net:'\"$net\"',host:'\"$host\"',path:'\"$path\"',tls:'\"tls\"'}' <<<{})
                ...
                is_url="$is_protocol://$uuid@$host:$is_https_port?encryption=none&security=tls&type=$net&host=$host&path=$path#sing-box-plus-$net-$host"
            }
            ...
            is_vmess_url=$(jq -c "{v:2,ps:\"sing-box-plus-${net}-$is_addr\",add:\"$is_addr\",port:\"$port\",id:\"$uuid\",aid:\"0\",net:\"$net\",type:\"$is_type\"$is_quic_add}" <<<{})
            is_url=vmess://$(echo -n $is_vmess_url | base64 -w 0)
        fi
        ;;
    ss)
        ...
        is_url="ss://$(echo -n ${ss_method}:${ss_password} | base64 -w 0)@${is_addr}:${port}#sing-box-plus-$net-${is_addr}"
    ...
    trojan)
        ...
        is_url="$is_protocol://$password@$is_addr:$port?type=tcp&security=tls&insecure=1&allowInsecure=1#sing-box-plus-$net-$is_addr"
    ...
    hy*)
        ...
        is_url="$is_protocol://$password@$is_addr:$port?alpn=h3&insecure=1&allowInsecure=1&pinSHA256=$is_sha256#sing-box-plus-$net-$is_addr"
    ...
    tuic)
        ...
        is_url="$is_protocol://$uuid:$password@$is_addr:$port?alpn=h3&insecure=1&allowInsecure=1&congestion_control=bbr#sing-box-plus-$net-$is_addr"
    ...
    reality)
        ...
        is_url="$is_protocol://$uuid@$is_addr:$port?encryption=none&security=reality&flow=$is_flow&type=$is_net_type&sni=$is_servername&pbk=$is_public_key&fp=chrome#sing-box-plus-$net-$is_addr"
        ;;
    anytls)
        is_can_change=(0 1 4)
        if [[ $is_anytls_domain ]]; then
            ...
            is_url="anytls://$password@$is_anytls_domain:$port#sing-box-plus-$net-$is_anytls_domain"
        else
            is_insecure=1
            is_info_show=(0 1 2 10 8 20)
            is_info_str=($is_protocol $is_addr $port $password tls true)
            is_url="anytls://$password@$is_addr:$port?insecure=1&allowInsecure=1#sing-box-plus-$net-$is_addr"
        fi
        ;;
    direct)
    ...
    socks)
        ...
        is_url="socks://$(echo -n ${is_socks_user}:${is_socks_pass} | base64 -w 0)@${is_addr}:${port}#sing-box-plus-$net-${is_addr}"
        ;;
    esac
...
url_qr() {
    ...
    if [[ $is_url ]]; then
        [[ $1 == 'url' ]] && {
            ...
            msg
            msg "如果无法正常显示或识别, 请复制上面的 URL 到二维码工具生成."
            footer_msg
        }
    else
        [[ $1 == 'url' ]] && {
...
main() {
    ...
    v | ver | version)
        ...
        msg "\n$(_green $is_core_name $is_core_ver) / $(_cyan $is_core_name-plus script $is_sh_ver) $is_caddy_ver\n"
        ;;
    h | help | --help)
        load help.sh
        show_help ${@:2}
